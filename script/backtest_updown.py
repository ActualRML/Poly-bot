...
SUPPORTED_ASSETS = list(SERIES.keys())

ENTRY_MINUTES_BEFORE = 90
...
async def fetch_events_for_asset(
    ...
    for _page in range(max_pages):
        try:
            ...
            logger.warning(f"[{symbol}] Gamma events fetch error: {e}")
            break
        ...
        for e in batch:
            ...
            e["_price_to_beat"] = float(price_to_beat)
            e["_final_price"]   = float(final_price)
            events.append(e)

        last_ed_str = batch[-1].get("endDate") or ""
        try:
            last_ed = datetime.fromisoformat(last_ed_str.replace("Z", "+00:00"))
            if last_ed < cutoff:
                break
        except Exception:
            pass
...
async def analyze_event(
    ...
    if not current:
        logger.debug(f"[{symbol}] skip {end_date.date()} — gagal fetch entry price")
        return None

    vol = await fetch_historical_realized_vol(symbol, session, at_time=entry_time, hours=4)
    if not vol or vol <= 0:
        vol = {"BTC": 0.40, "ETH": 0.55, "SOL": 0.70, "XRP": 0.60}.get(symbol, 0.50)
...
def print_report(results: list[dict], calibration: dict, days: int):
    print("=" * 60)
    print(f"BACKTEST UP/DOWN DAILY — {days} hari terakhir")
    print(f"Total events dianalisis: {len(results)}")
    print(f"Reference: eventMetadata.priceToBeat (Gamma API)")
    print("=" * 60)
...
async def main(days: int, asset_filter: Optional[str]):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for symbol in assets:
            print(f"Fetching {symbol} Up/Down Daily events (last {days} days)...")
            evts = await fetch_events_for_asset(symbol, session, days, gamma)
            print(f"  -> {len(evts)} events ditemukan")
            all_events.extend(evts)

        if not all_events:
            print("Tidak ada events ditemukan. Coba tambah --days.")
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--asset", type=str, default=None, help=f"Filter asset: {', '.join(SERIES.keys())}")
